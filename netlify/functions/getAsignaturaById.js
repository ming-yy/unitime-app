...
export default async (req, context) => {
    ...
    try {
        ...
        const asignatura = await collection.findOne({_id: new ObjectId(_id)}, function (err, result) {
            if (err) {
        ...
        console.log("asignatura found:", asignatura);

        if (asignatura) {
            return new Response(JSON.stringify(asignatura), { status: 200 });
        } else {
            return new Response("asignatura not found", { status: 404 });
        }
