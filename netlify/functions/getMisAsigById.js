...
export default async (req, context) => {
    ...
    try {
        ...
        const collection = database.collection('misAsignaturas');

        const misAsig = await collection.findOne({_id: new ObjectId(_id)}, function (err, result) {
            if (err) {
        ...
        console.log("Asignaturas found:", misAsig);

        if (misAsig) {
            return new Response(JSON.stringify(misAsig), { status: 200 });
        } else {
